// group m3137
// solutions with MyScanner
//     HexDec2 - ReverseHexDec2.java
//     Min2 (mod for 3 homework)- ReverseMin2.java
//     WordStat (solution for fourth task without modifications)- WordStatInput.java
//     Words (mod for 4 homework)- WordStatWords.java
...
public class MyScanner {
    ...
    public boolean isWordChar(char symbol) {
    ...
    public boolean hasNextTokenInLine() throws IOException {
        if (!letterDashApoMode) {
            while (Character.isWhitespace(charBuffer[currentPosition]) &&
                    currentBufferSize != -1) {
                if (isLineSeparator(charBuffer[currentPosition])) {
                    return false;
                }
                nextChar();
            }
        } else {
            while (!isWordChar(charBuffer[currentPosition]) &&
                    currentBufferSize != -1) {
                if (isLineSeparator(charBuffer[currentPosition])) {
                    return false;
                }
